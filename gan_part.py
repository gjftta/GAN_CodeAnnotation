...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        '''
        Every epoch train G for 5 times & train D for 1 times
        '''
        # Train G
        ...
        for i in range(5):
            ...
            for src_sent, tgt_sent in train_dataloader:
                ...
                writer.add_scalar('train_loss', losses / (step + 1), epoch * 5 * 16 + i * 16 + step)

                if step % 5 == 0:
                    end_time = timer()
                    log_train.debug('epoch:{0}\tG_epoch:{1}\tstep: {2}\tloss: {3:.3f}\ttime: {4:.4f}'.format(epoch, i, step, losses / (step + 1), end_time - start_time))
                    start_time = timer()
